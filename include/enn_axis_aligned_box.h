...
namespace enn
{
    ...
    public:
		...
		{
            ENN_ASSERT( (min.x <= max.x && min.y <= max.y && min.z <= max.z) );
		...
		*/
		...
		{
			ENN_ASSERT( (m_extent == EXTENT_FINITE) );
			...
			if (!m_corners)
				m_corners = ENN_MALLOC_ARRAY_T(Vector3, 8);
