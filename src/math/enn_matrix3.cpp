...
namespace enn
{
	const float Matrix3::EPSILON = (float)1e-06;
	const Matrix3 Matrix3::ZERO(0,0,0,0,0,0,0,0,0);
	const Matrix3 Matrix3::IDENTITY(1,0,0,0,1,0,0,0,1);
	const float Matrix3::s_svdEpsilon = (float)1e-04;
	const unsigned int Matrix3::s_svdMaxIterations = 32;
	...
	void Matrix3::_bidiagonalize(Matrix3& kA, Matrix3& kL,
		Matrix3& kR)
	{
		float afV[3], afW[3];
	...
	void Matrix3::_golubKahanStep (Matrix3& kA, Matrix3& kL,
		Matrix3& kR)
	{
		...
		// adjust right
		float fY = kA[0][0] - (Math::fabs(fRoot1-fT22) <=
		...
		Matrix3& kR) const
	{
		...
		for (unsigned int i = 0; i < s_svdMaxIterations; i++)
		{
			...
			bool bTest1 = (Math::fabs(kA[0][1]) <=
				s_svdEpsilon*(Math::fabs(kA[0][0])+Math::fabs(kA[1][1])));
			bool bTest2 = (Math::fabs(kA[1][2]) <=
				s_svdEpsilon*(Math::fabs(kA[1][1])+Math::fabs(kA[2][2])));
			if ( bTest1 )
			{
				if ( bTest2 )
				{
	...
	float Matrix3::_maxCubicRoot(float afCoeff[3])
	{
		...
		const float fOneThird = (float)1.0/(float)3.0;
		...
		if ( fPoly < 0.0 )
		{
			...
			float fTmp = 1.0f+Math::fabs(afCoeff[1]);
			if ( fTmp > fX )
				fX = fTmp;
	...
	float Matrix3::spectralNorm() const
	{
		...
		float fRoot = _maxCubicRoot(afCoeff);
		float fNorm = Math::sqrt(fPmax*fRoot);
		return fNorm;
	}

	void Matrix3::toAngleAxis(Vector3& rkAxis, float& rfRadians) const
	{
	...
	void Matrix3::fromAngleAxis(const Vector3& rkAxis, float fRadians)
	{
	...
	void Matrix3::_tridiagonal(float afDiag[3], float afSubDiag[3])
	{
	...
	bool Matrix3::_qLAlgorithm(float afDiag[3], float afSubDiag[3])
	{
		...
		Vector3 akEigenvector[3]) const
	{
		...
		float afSubDiag[3];
		kMatrix._tridiagonal(afEigenvalue,afSubDiag);
		kMatrix._qLAlgorithm(afEigenvalue,afSubDiag);
