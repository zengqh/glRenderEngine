...
namespace enn
{
	...
	void Quaternion::fromRotationVector(const Vector3 &rotateFrom, const Vector3& rotateTo)
	{
		Vector3 p1, p2;
		float alpha;

		p1 = rotateFrom.normalisedCopy();
		p2 = rotateTo.normalisedCopy();

		alpha = p1.dotProduct(p2);
		...
		if( alpha == float(-1.0))
		{
			...
			if(p1[0] != p1[1] || p1[0] != p1[2])
				v = Vector3(p1[1], p1[2], p1[0]);
			else
				v = Vector3(-p1[0], p1[1], p1[2]);

			v -= p1 * p1.dotProduct(v);
			v.normalise();

			fromAngleAxis(float(3.1415926), v);
		}

		p1 = p1.crossProduct(p2).normalise();

		fromAngleAxis(float(acos(alpha)), p1);
	}
