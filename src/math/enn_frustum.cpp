...
namespace enn
{
	...
	{
		m_dirty.set_flag( DIRTY_VIEW | DIRTY_PROJECTION );
	}
	...
	Vector3 Frustum::getLookDir() const
	{
		return -m_matWorld.getAxisZ();
	}

	Vector3 Frustum::getUpDir() const
	{
		return m_matWorld.getAxisY();
	}
	...
	void Frustum::_update()
	{
		...
		// view更新
		if ( m_dirty.test_flag(DIRTY_VIEW) )
		{
			viewOrProjDirty = true;

			m_matViewRD = m_matView;

			m_dirty.reset_flag(DIRTY_VIEW);
		}

		// proj更新
		if ( m_dirty.test_flag(DIRTY_PROJECTION) )
		{
			viewOrProjDirty = true;

			if ( m_projType == PT_PERSPECTIVE )
				_makePerspectiveProj();
				//Math::makeGLPerspective(m_matProj, m_fov, m_aspect, m_zNear, m_zFar);
			...
			m_dirty.reset_flag(DIRTY_PROJECTION);
		}
	...
	{
		m_dirty.set_flag(DIRTY_VIEW);
	}

	void Frustum::_setProjectionDirty()
	{
		m_dirty.set_flag(DIRTY_PROJECTION);
	}
}